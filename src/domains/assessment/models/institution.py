...
class Institution(FullBaseModel):
    ...
    users = relationship(
        "UserInstitution", back_populates="institution", cascade="all, delete-orphan"
    )

    __table_args__ = (
        UniqueConstraint("name", name="uq_institution_name"),
        UniqueConstraint("code", name="uq_institution_code"),
        CheckConstraint("total_users >= 0", name="check_total_users_nonnegative"),
        CheckConstraint(
            "total_assessments >= 0", name="check_total_assessments_nonnegative"
        ),
        CheckConstraint("total_courses >= 0", name="check_total_courses_nonnegative"),
        CheckConstraint("total_students >= 0", name="check_total_students_nonnegative"),
    )
...
class UserInstitution(FullBaseModel):
    __tablename__ = "user_institution"

    user_id = Column(PG_UUID(as_uuid=True), ForeignKey("user.id"), nullable=False)
    institution_id = Column(
        PG_UUID(as_uuid=True), ForeignKey("institution.id"), nullable=False
    )
    role = Column(String(50), nullable=False, default="student")
    is_active = Column(Boolean, default=True)

    # user = relationship("User", back_populates="institution_links")
